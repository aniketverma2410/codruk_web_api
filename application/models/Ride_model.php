...
// get differ type unit
    public function getDataViaType($table,$type){  
     ...
     public function get_pending_rides(){  

              $this->db->order_by('id','desc');  
       return $this->db->get_where('ride_master',array('status'=>1))->result_array();
    }

     public function get_pending_rides_vehicles($vehicle_id){  

              $this->db->order_by('id','desc');  
       return $this->db->get_where('ride_master',array('vehicle_id'=>$vehicle_id,'status'=>1))->result_array();
    }

     public function get_cancel_rides(){  

              $this->db->order_by('id','desc');  
       return $this->db->get_where('ride_master',array('status'=>2))->result_array();
    }

     public function get_ride_details($id){  

              
       return $this->db->get_where('ride_master',array('id'=>$id))->row_array();
    }
